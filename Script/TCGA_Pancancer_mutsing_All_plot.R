setwd("/Users/changc11/Documents/MRPPanCancer/RESULTS")
...
bp+ scale_fill_manual(values=all.colors1)+ theme(axis.text.x = element_text( size=8, angle=90))
pdf(file="Pan_cancer_All_plotSignatures1.pdf",width=5, height=6,compress=TRUE,useDingbats=F)
bp+ scale_fill_manual(values=all.colors1)+xlab("Mutation Type Probability (%)")+ theme(axis.text.x = element_text( size=10, angle=90))+scale_x_discrete(labels = c("WT", "P53-mut", "HMD", "P53-mut/HMD"))
dev.off()
...
all.colors <-c("#00441b","#006d2c","#238b45","#41ae76","#66c2a4","#99d8c9","#ccece6","#ffeda0","#fed976","#feb24c","#fd8d3c","#fc4e2a","#e31a1c","#bd0026","#08306b","#08519c","#2171b5","#4292c6","#9ecae1","#c6dbef","#deebf7","#f7fbff","#ffffcc","#e7e1ef","#d4b9da","#c994c7","#df65b0","#e7298a","#ce1256","#980043","#67001f")
all.sigs <-c("Signature.14","Signature.28","Signature.23","Signature.12","Signature.27","Signature.30","Signature.5","Signature.16","Signature.18","Signature.unknown","Signature.17","Signature.19","Signature.8","Signature.25","Signature.10","Signature.6","Signature.20","Signature.15","Signature.21","Signature.26","Signature.1","Signature.9","Signature.3","Signature.22","Signature.2","Signature.29","Signature.24","Signature.13","Signature.4","Signature.11","Signature.7")
mydf_Group_4_foldchange$group_new <- factor(mydf_Group_4_foldchange$group_new, levels = all.sigs)


pdf(file="Pan_cancer_All_plotSignatures_foldchange_P53_MRP_del_P53.pdf",width=3, height=5,compress=TRUE,useDingbats=F)
ggplot(mydf_Group_4_foldchange, aes(x=-P53_MRP_del_P53_ratio_log, y=group_new, fill=group_new)) +
  geom_bar(stat="identity", position="dodge") +
  guides(fill=FALSE)+ scale_fill_manual(values=all.colors)+
  xlab("Foldchange") + ylab("Signature")+ ggtitle("P53-mut/HMD v/s P53-mut")+theme_minimal()
dev.off()

pdf(file="Pan_cancer_All_plotSignatures_foldchange_MRP_del_WT.pdf",width=3, height=5,compress=TRUE,useDingbats=F)
ggplot(mydf_Group_4_foldchange, aes(x=-MRP_del_WT_ratio_log, y=group_new, fill=group_new)) +
  geom_bar(stat="identity", position="dodge") +
  guides(fill=FALSE)+ scale_fill_manual(values=all.colors)+
  xlab("Foldchange") + ylab("Signature")+ ggtitle("HMD v/s WT")+theme_minimal()
dev.off()
