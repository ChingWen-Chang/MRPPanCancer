forestPlot1<-function (mafCompareRes, pVal = 0.05, fdr = NULL, color = NULL, 
          geneFontSize = 1.2, titleSize = 1.2, lineWidth = 2.2) 
{
  ...
  m.sigs$log10OR_high = ifelse(test = is.infinite(m.sigs$log10OR_high), 
                               yes = ifelse(test = m.sigs$log10OR_high > 0, yes = lim, 
                                            no = -lim), no = m.sigs$log10OR_high)
  m.sigs$log10OR_low = ifelse(test = is.infinite(m.sigs$log10OR_low), 
                              yes = ifelse(test = m.sigs$log10OR_low > 0, yes = lim, 
                                           no = -lim), no = m.sigs$log10OR_low)
  if (!is.null(color)) {
    color = color
  ...
  graphics::layout(mat = matrix(c(1, 2, 3, 4, 5, 5, 5, 5), 
                                byrow = TRUE, ncol = 4, nrow = 2), widths = c(4, 1, 1), 
                   heights = c(6, 1.2))
  par(mar = c(3, 1, 3, 5))
  plot(rep(0, nrow(m.sigs)), 1:nrow(m.sigs), xlim = c((-lim+1), 
                                                      (lim-1)), axes = FALSE, pch = NA, xlab = "", ylab = "", ylim = c(0.5, 
                                                                                                                   nrow(m.sigs)))
  ...
  points(m.sigs$log10OR, 1:nrow(m.sigs), pch = 16, cex = 0.5 * 
           (lineWidth))
  axis(side = 1, at = c((-lim+1), 0, (lim-1)), lwd = 2.2, font = 1, 
  ...
  mtext(text = "Log odds ratio", side = 1, line = 2, font = 1, 
        cex = 0.7 * (titleSize))
  par(mar = c(3, 0, 3, 0))
  plot(rep(0, nrow(m.sigs)), 1:nrow(m.sigs), xlim = c(0, 1), 
       axes = FALSE, pch = NA, xlab = "", ylab = "", ylim = c(0.5, 
                                                              nrow(m.sigs)))
  text(x = 0.5, y = 1:nrow(m.sigs), labels = as.numeric(unlist(m.sigs[, 
                                                                      2])), adj = 0, font = 1, cex = 1.4 * (geneFontSize))
  ...
  plot(rep(0, nrow(m.sigs)), 1:nrow(m.sigs), xlim = c(0, 1), 
       axes = FALSE, pch = NA, xlab = "", ylab = "", ylim = c(0.5, 
                                                              nrow(m.sigs)))
  text(x = 0.5, y = 1:nrow(m.sigs), labels = as.numeric(unlist(m.sigs[, 
                                                                      3])), adj = 0, font = 1, cex = 1.4 * (geneFontSize))
  ...
  legend(x = "top", legend = names(color), lwd = lineWidth, 
         ...
         cex = 1.1 * (titleSize), horiz = TRUE, text.font = 1, 
         xpd = TRUE, pch = 16)
}
