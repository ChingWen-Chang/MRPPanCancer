Pan_cancer_TP53_mutation_group_all<- read.delim("/Users/changc11/Documents/MSK-IM/MSKCC_mutation/Pan_cancer_TP53_mutation_group_all_MSKCC.txt",header = T)
Pan_cancer_WT_Sing_Patient_100<- read.delim("/Users/changc11/Documents/MSK-IM/MSKCC_mutation/Pan_cancer_WT_Sing_Patient_25_MSKCC.txt",header = T)
Pan_cancer_MD.LD_Sing_Patient_100 <- read.delim("/Users/changc11/Documents/MSK-IM/MSKCC_mutation/Pan_cancer_MD.LD_Sing_Patient_25_MSKCC.txt", header=T, row.names=1)


setwd("/Users/changc11/Documents/MSK-IM/MSKCC_mutation")
library(ggplot2)
library(dplyr)
library(ggpubr)
...
Pan_cancer_Sing_all<-cbind(group_new=Pan_cancer_Sing$group_new, Pan_cancer_Sing[,104:134])
...
Pan_cancer_Sing_all_sum_t_1 <- read.delim("/Users/changc11/Documents/MSK-IM/MSKCC_mutation/Pan_cancer_Sing_all_sum_t.txt")
...
pdf(file="Pan_cancer_All_plotSignatures1_MSKCC.pdf",width=6, height=4,compress=TRUE,useDingbats=F)
bp+ scale_fill_manual(values=all.colors1)
dev.off()
...
library(dplyr)
library(plyr)
m1 = ddply(Pan_cancer_Sing_all_sum_t_1, .(group_new), summarize, ratio=sum(Signature))
#write.table(m1,file="Pan_cancer_Sing_all_sum_ratio.txt",quote = F,sep = "\t")
...
pdf(file="Pan_cancer_All_plotSignatures_foldchange_P53_MRP_del_WT_MSKCC.pdf",width=5, height=3,compress=TRUE,useDingbats=F)
...
pdf(file="Pan_cancer_All_plotSignatures_foldchange_P53_MRP_del_P53_MSKCC.pdf",width=5, height=3,compress=TRUE,useDingbats=F)
ggplot(mydf_Group_4_foldchange, aes(group_new, P53_MRP_del_P53_ratio_log, fill=ifelse(P53_MRP_del_P53_ratio_log>0,"P53_MRP_del","P53"))) +
  geom_bar(stat="identity") +
  scale_fill_manual(values=c("blue", "red"), name="P53 or P53_MRP_del") +
  scale_y_continuous(labels = function(y) y )+ theme(axis.text.x = element_text(face="bold", color="black", size=8, angle=45))
dev.off()

pdf(file="Pan_cancer_All_plotSignatures_foldchange_MRP_del_WT_MSKCC.pdf",width=5, height=3,compress=TRUE,useDingbats=F)
...
p + stat_compare_means(method = "anova", label.y = 1.2)+      # Add global p-value
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = ".all.", data =Pan_cancer_Sing_t)           
